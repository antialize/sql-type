...
use sql_ast::{Span, DataType, Issue, parse_statements};
use crate::{Type, RefOrVal, type_::FullType};
...
fn parse_column<'a>(data_type: DataType<'a>, identifier_span: Span,_issues: &mut Vec<Issue>) -> Column<'a> {
    ...
    let type_ = match data_type.type_ {
        ...
        sql_ast::Type::Enum(e) => Type::Enum(RefOrVal::Val(e.into_iter().map(|(s,_)| s).collect())),
